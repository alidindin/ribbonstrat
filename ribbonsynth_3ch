...
int fsrVals[] = {0, 0, 0};
int fsrPins[] = {PIN_FSR_1, PIN_FSR_2, PIN_FSR_3};
...
void readSensors() {
  for (int i=0; i<NUM_STRINGS; i++) {
    ...
    */
    ...
    if (fsrVal > PIEZO_THRESHOLD_ON) {
      int v_new = fsrVal;
      for (int sample=0; sample<FSR_SAMPLES; sample++){
        fsrVal = analogRead(fsrPins[i]);
        if (fsrVal > v_new){
          v_new = fsrVal;
        }
      }
      fsrVals[i] = v_new;
      fsrVals[i] = map(fsrVals[i], 0, 1023, 0, 127);
      fsrVals[i] = constrain(fsrVals[i], minVelocity, 127);
    }
  ...
  for (int i=0; i< NUM_STRINGS; i++) {
    ...
    if (fsrVals[i]){
      ...
      if (!stringActive[i]) {
        ...
        if (fretTouched[i] > 0) activeNotes[i]->init(noteFretted[i], fsrVals[i], millis(), true);
      
        else activeNotes[i]->init(noteFretted[i], fsrVals[i], millis(), false);
